...
try:
    ...
    for imagePath in imagePaths:
        ...
        if imagePath[-6] is '/':
            imagePath = imagePath[:-5] + '0' + imagePath[-5:]

        #print("BP2", imagePath)   



        with open(os.path.dirname(imagePath) + '/keypoints' + imagePath[-6:-4] + '.json', "w") as fp:
            json.dump(keypoints_numpy.tolist(), fp, indent=4)

        #für Verwendung mit Array
        keypoints_array = np.append(keypoints_array, keypoints_numpy)
        #print("KP", keypoints_array)
