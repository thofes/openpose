...
try:
    ...
    opWrapper.start()
    print("ts ", args[0].image_dir)
    with open(args[0].image_dir, "r") as fp:
        data = json.load(fp)
  
    pfad = np.array(data["path"])
    ...
    for imagePath in imagePaths:
        ...
        for i, kp in enumerate(datum.poseKeypoints):
            print("xhals : ", datum.poseKeypoints[i][1][0])
            if datum.poseKeypoints[i][1][0] > 150 and datum.poseKeypoints[i][1][0] < 250:
              print("OK", i)
              x1 = datum.poseKeypoints[i][21][0]
              x2 = datum.poseKeypoints[i][24][0]
              y1 = datum.poseKeypoints[i][21][1]
              y2 = datum.poseKeypoints[i][24][1]
              print(datum.poseKeypoints[i][21])
              print(datum.poseKeypoints[i][24])
        ...
        print(x_crop, y_crop)
        print(x_dif, y_dif)
        list_keypoints.append(x_dif)
        list_keypoints.append(y_dif)

        print("Finale X = ", x_center[0]+x_dif)
        print("Finale Y = ", y_center[0]+y_dif)
        ...
        keypoints_numpy = datum.poseKeypoints
